...
DATA_PATH = '/Users/premgalani/Desktop/VTION_Influencer_Data/social_influencers_recategorized.csv'
...
scatter_df = df[
    ['handle_name', 'event_count', 'unique_user_count', 'total_brand_collabs']
].copy()
...
def update_dashboard(selected_handle):
    ...
    er = int(row['event_rank'])
    fl = int(row['followers'])

    # Build KPI cards
    cards = [
        html.Div(
            [html.H3("Event Count"), html.P(f"{ev:,}")],
            style={'border':'1px solid #ccc','padding':'10px',
                   'borderRadius':'5px','width':'19%'}
        ),
        html.Div(
            [html.H3("Unique Users"), html.P(f"{uu:,}")],
            style={'border':'1px solid #ccc','padding':'10px',
                   'borderRadius':'5px','width':'19%'}
        ),
        html.Div(
            [html.H3("Followers"), html.P(f"{fl:,}")],
            style={'border':'1px solid #ccc','padding':'10px',
                   'borderRadius':'5px','width':'19%'}
        ),
        html.Div(
            [html.H3("Total Brand Collabs"), html.P(f"{tb:,}")],
            style={'border':'1px solid #ccc','padding':'10px',
                   'borderRadius':'5px','width':'19%'}
        ),
        html.Div(
            [html.H3("Event Rank"), html.P(f"{er}")],
            style={'border':'1px solid #ccc','padding':'10px',
                   'borderRadius':'5px','width':'19%'}
        ),
    ]
    ...
    scatter_fig.add_hline(
        y=uu, line_dash="dash",
        annotation_text="Selected", annotation_position="right"
    )

    # Brand Collaborations Treemap
    tree_fig = px.treemap(
        bc_df, path=['category','brand'],
